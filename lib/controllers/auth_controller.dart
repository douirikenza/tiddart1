...
class AuthController extends GetxController {
  ...
  Future<void> login(String email, String password) async {
    try {
      ...
      var userData = await getCurrentUserData();
      if (userData == null) {
        throw 'Compte non trouvé';
      }

      // Vérifier si c'est un compte artisan
      if (userData['role'] == 'artisan') {
        ...
        if (userData['isApproved'] == false) {
          ...
          throw 'Votre compte artisan est en attente d\'approbation.';
        }
        ...
        if (userData['isActive'] == false) {
          ...
          Get.snackbar(
            "Compte désactivé",
            'Votre compte artisan a été désactivé. Veuillez contacter l\'administrateur.',
            snackPosition: SnackPosition.BOTTOM,
            backgroundColor: Colors.red.shade100,
            colorText: Colors.brown,
            duration: const Duration(seconds: 4),
          );
          throw 'Votre compte artisan a été désactivé.';
        }
      }
  ...
  Future<void> signup(String name, String email, String password, bool isArtisan) async {
    try {
      ...
      await _firestore.collection('users').doc(result.user!.uid).set({
        ...
        'isApproved': isArtisan ? false : true, // Les artisans doivent être approuvés
        'isActive': true,
      ...
      // Message spécial pour les artisans
      if (isArtisan) {
        Get.snackbar(
          "Inscription réussie",
          "Votre compte artisan est en attente d'approbation par l'administrateur.",
